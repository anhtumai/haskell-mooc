...
combine blender (Picture f1) (Picture f2) = Picture f
  where
    f coord = blender (f1 coord) (f2 coord)
...
instance Transform Zoom where
  apply (Zoom scale) (Picture base) = Picture f
    where
      f (Coord x y) = base (Coord (div x scale) (div y scale))

data Flip = FlipX | FlipY | FlipXY
  deriving (Show)

instance Transform Flip where
  apply FlipX (Picture base) = Picture f
    where
      f (Coord x y) = base (Coord (- x) y)
  apply FlipY (Picture base) = Picture f
    where
      f (Coord x y) = base (Coord x (- y))
  apply FlipXY (Picture base) = Picture f
    where
      f (Coord x y) = base (Coord y x)
