...
data UpDown = Up Int | Down Int deriving Show
...
average' xs ys = map (\(x,y) -> (x+y)/2) $ zip xs ys

invert' :: [Double] -> [Double]
invert' xs = map (\x -> (1-x) ) xs
...
toList Empty = []
toList (Pair key value kp) = (key,value) : toList kp
...
toNat z | z < 0 = Nothing
toNat 0 = Just Zero
toNat z = Just (PlusOne ( fromJust (toNat (z-1))))
...
toBin 1 = I End
toBin 0 = O End
toBin x = if mod x 2 == 1 then I (toBin (div x 2)) else O (toBin (div x 2))
