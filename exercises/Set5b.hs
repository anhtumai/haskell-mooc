...
treeMax Empty = 0
treeMax (Node a l r) = max (max (treeMax l) (treeMax r)) a
...
cull val (Node a l r) | a == val = Empty
cull val (Node a l r) = Node a (cull val l) (cull val r)
...
isOrdered (Node a l r) = (isNothing rootL || a > fromJust rootL) && (isNothing rootR || a < fromJust rootR) && isOrdered l && isOrdered r
  where
    rootL = valAtRoot l
    rootR = valAtRoot r
...
walk (step : steps) (Node a l r)
  | step == StepL = walk steps l
  | step == StepR = walk steps r
...
set (step : steps) val (Node a l r)
  | step == StepL = Node a (set steps val l) r
  | step == StepR = Node a l (set steps val r)
...
search val (Node a _ _) | val == a = Just []
search val Empty = Nothing
search val (Node a l r)
  | isJust searchL = Just (StepL : fromJust searchL)
  | isJust searchR = Just (StepR : fromJust searchR)
  | otherwise = Nothing
  where
    searchL = search val l
    searchR = search val r
